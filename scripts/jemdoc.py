    ...
    while 1:  # wait for EOF.
        ...
        # look for blocks.
            ...
            if infoblock:
                ...
                if len(g) in (0, 1):  # info block.
                ...
                elif len(g) >= 4 and len(g) <= 7 and g[1] == 'img_left':
                    # handles
                    ...
                    out(f.outf, '&nbsp;</td>\n<td align="left">')
                    imgblock = True
                elif len(g) >= 8 and g[1] == 'img_left':
                    # handles
                    # {}{img_left}{source}{alttext}{width}{height}{linktarget}{img_left}{source}{alttext}{width}{height}{linktarget}.
                    # g += [''] * (7 - len(g))
                    ...
                    out(f.outf, '<table class="imgtable"><tr><td>\n')
                    if g[6]:
                        out(f.outf, '<a target="_blank" href="%s">' % g[6])
                    out(f.outf, '<img src="%s"' % g[2])
                    out(f.outf, ' alt="%s"' % g[3])
                    if g[4]:
                        out(f.outf, ' width="%s"' % g[4])
                    ...
                    if g[12]:
                        out(f.outf, '<a href="%s">' % g[12])
                    out(f.outf, '<img src="%s"' % g[8])
                    out(f.outf, ' alt="%s"' % g[9])
                    if g[10]:
                        out(f.outf, ' width="%s"' % g[10])
                    if g[11]:
                        out(f.outf, ' height="%s"' % g[11])
                        out(f.outf, ' />')
                    if g[12]:
                        out(f.outf, '</a>')

                    if g[18]:
                        out(f.outf, '<a href="%s">' % g[18])
                    out(f.outf, '<img src="%s"' % g[14])
                    out(f.outf, ' alt="%s"' % g[15])
                    if g[16]:
                        out(f.outf, ' width="%s"' % g[16])
                    if g[17]:
                        out(f.outf, ' height="%s"' % g[17])
                    out(f.outf, ' />')
                    if g[18]:
                        out(f.outf, '</a>')
                    ...
                    # if g[24]:
                    #     out(f.outf, '<a href="%s">' % g[18])
                    # out(f.outf, '<img src="%s"' % g[20])
                    # out(f.outf, ' alt="%s"' % g[21])
                    # if g[22]:
                    #     out(f.outf, ' width="%s"' % g[22])
                    # if g[23]:
                    #     out(f.outf, ' height="%s"' % g[23])
                    # out(f.outf, ' />')
                    # if g[24]:
                    #     out(f.outf, '</a>')
